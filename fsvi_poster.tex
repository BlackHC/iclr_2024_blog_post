...
\begin{columns}
  ...
  \begin{tldrbox}
    ...
    Formally, for distributions $\mathrm{q}(\boldsymbol{\Theta})$ and $\mathrm{p}(\boldsymbol{\Theta})$ over a random variable $\boldsymbol{\Theta}$ and a stochastic mapping $Y = \mathrm{f}(\boldsymbol{\Theta})$, the Kullback-Leibler DPI states:
...
%   \begin{multicols}{2}
  \begin{backgroundbox}[title={Chain Rule of the \includegraphics[width=1em]{leafy-green.png} Divergence}]
    ...
    &\begin{aligned}
      & = \Kale{\qof{\Y_n \given \Y_{n-1}, ...}}{\pof{\Y_n \given \Y_{n-1}, ...}} \\
      & \quad + \Kale{\qof{\Y_{n-1}, ...}}{\pof{\Y_{n-1}, ...}} \\
      = ... & = \sum_{i=1}^n \Kale{\qof{\Y_i \given \Y_{i-1}, ...}}{\pof{\Y_i \given \Y_{i-1}, ...}}.
    \end{aligned}
    \end{align*}
    This also yields a \textbf{chain inequality}:
    \begin{align*}
    \Kale{\qof{\Y_n,...}}{\pof{\Y_n,...}} &\ge \Kale{\qof{\Y_{n-1},...}}{\pof{\Y_{n-1},...}} \\
    \ge ... &\ge \Kale{\qof{\Y_1}}{\pof{\Y_1}},
    \end{align*}
