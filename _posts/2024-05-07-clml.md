...
If our model fulfills exchangeability, that is the order of the $$\x_n$$ does not matter, we could take an expectation over all permutations of the data, obtaining the **joint cross-entropy**:

$$
\CrossEntropy{\pdata{\X_1, ...,\X_n}}{\pof{\X_1, ... \X_n \given \h}}.
$$

This leads to the most common extension that is often connected to Occam's razor. Before we try to understand this expression better, we should consider the space of alternatives.

Indeed, this is not the only way to extend the previous definitions. For example, we could take the average of the likelihoods for individual data points:
...
From the perspective of Occam's razor, this is not very principled, and we can do better: we have previously used information theory as our tool of choice, and actually we prefer small $$\Hof{\x \given \h}$$ and on average over the data distribution, this yields the cross-entropy:

$$\CrossEntropy{\pdata{\X}}{\pof{\X \given \h}} = \E{\pdata{\x}}{-\log \pof{\x \given \h}}.$$

This is the average number of bits needed to encode the data using the model's probability distribution. Because we only use the marginal distribution, I refer to this as the **marginal cross-entropy**.

<aside class="box-note l-body" markdown="1">
This marginal cross-entropy is also a commonly-used loss function in machine learning, and minimizing it is equivalent to minimizing the Kullback-Leibler divergence between the model's distribution and the true data distribution. Sometimes, it is also referred to as the **negative log-likelihood (NLL)** or the negative log-likelihood loss.
</aside>

Obviously, this marginal cross-entropy and the average likelihood are not the same. Using the convexity of the negative logarithm and Jensen's inequality, we can show that the cross-entropy is always larger than the negative logarithm of the average likelihood:
...
Crucially, these summary statistics are very different from the first using the joint distribution: when we use the marginal cross-entropy and related summary statistics to measure the performance of the model, we operate on the "prior" distribution and not the posterior distribution of the model parameters. Intuitively, this follows from not conditioning on any additional data. This is a very important point.

The joint distribution captures something else, however. We can see this most clearly when using the chain rule. For statement above before the taking the expectation, we can write using the chain rule:
...
$$
\begin{aligned}
& \CrossEntropy{\pdata{\X}}{\pof{\Dany \given \h}} = \\
...
Each of the terms in the sum is a marginal cross-entropy conditioned on the previous data points. This is a very different quantity from the marginal cross-entropy, which is only conditioned on the prior distribution of the data. Indeed, we recognize the original marginal cross-entropy as the first term in the sum.
...
The marginal cross-entropy, on the other hand, measures the performance of the model on a fixed data distribution, which is a more static view of a model's performance.
...
2. If we care about the model's "generalization" performance *after training on $$N-1$$ data points* and without further adaption, the marginal cross-entropy as in the last term is the most relevant quantity: 
  
    $$\CrossEntropy{\pdata{\X_N \given \X_{N-1}, ..., \X_1}}{\pof{\X_N \given \X_1, \X_2, \ldots, \X_{N-1}, \h}}$$
  
    It measures the model's performance on the last data point after having seen all previous data points as "leave-one-out" metric. Indeed, it is equivalent to [leave-one-out cross-validation](https://www.wikiwand.com/en/Cross-validation_(statistics)#Leave-one-out_cross-validation).
    
3. More generally, it is equivalent to cross-validation when we hold out more than one data point for evaluation:

    $$ \CrossEntropy{\pdata{\X_{N-k+1}, ..., \X_N \given \X_1, ..., \X_{N-k}}}{\pof{\X_{N-k+1}, ..., \X_N \given \X_1, ..., \X_{N-k}, \h}}. $$

3. On the other hand, if we care about the model's performance as online learner, or let's say *in-context learner in the case of LLMs*, a joint cross-entropy is the more relevant quantity because it measures the adaptability of the model to new data points. 

4. Obviously, we would not use the joint cross-entropy on the prior but want to condition on some data first to be closer to the actual use-case of the model, which will have been (pre-)trained on some data. As such, we would be interested in estimating a *conditional joint cross-entropy*.

5. All these quantities are equally valid as far as Occam's razor is concerned---we have already connected Occam's razor to MLE and MAP after all, which are often not even considered as properly Bayesian. 
...
We will look at the paper in detail next, this discussion already brought us to the main point of the paper: 
...
## Background on Bayesian Concepts
