...
The authors demonstrate that the CLML is correlated with generalization for a range of data sizes, unlike the marginal likelihood, and provides promising performance for deep kernel hyperparameter learning and neural architecture search. 
...
After this tour through the various concepts, let's take a look at different experiments that show that the hypothetical considerations we have discussed are indeed relevant in practice.
...
Model Selection vs Hypothesis testing: we want H(D, M) to be small for hypothesis testing.
for model selection, we want H(D|M) to be small  


---

TK remove this again TK 
