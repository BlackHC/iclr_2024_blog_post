...
The joint distribution captures something else, however. 
We can see this most clearly when using the chain rule.
Using the chain rule on joint marginal likelihood yields:
...
Each term is a *conditional marginal information* conditioned on the previous data points.
Similarly, when we take an expectation over the data distribution, we obtain a chain of marginal cross-entropies:
...
Each of the terms in the sum is of a conditional marginal cross-entropy conditioned on the previous data points. This is a very different quantity from the marginal cross-entropy, which is not conditioned on any of the data, except for the first term, which we recognize the original marginal cross-entropy.
...
Many of these thoughts are captured in the following visualization:
...
*The relationship between the conditional and joint marginal cross-entropies and information as a function of dataset size.* The left plot shows the conditional marginal cross-entropy (blue line) for a fictious multi-class classification problem with 10 classes. As the dataset size increases, the conditional marginal cross-entropy decreases and converges to the best achievable loss for the given model hypothesis \(\h\). The area under the conditional marginal cross-entropy curve (shaded in orange) as the sum of the conditional marginal cross-entropies over the dataset is equal the joint marginal cross-entropy by virtue of the chain rule. 

The right plot illustrates the conditional marginal log likelihood (green line) for the same problem. The conditional marginal log likelihood is a noisy estimate of the conditional marginal cross-entropy, as it is computed on individual data points. The area under the conditional marginal log likelihood curve (shaded in red) represents the marginal likelihood, which is the product of the conditional marginal likelihoods over the dataset.

In the infinite data limit, the marginal cross-entropy and conditional marginal log likelihood converge to the same value, and the joint cross-entropy and marginal likelihood become equivalent up to a constant factor. However, in the low-data regime, these quantities can differ significantly, leading to different rankings of model hypotheses or hyperparameter choices.
</figcaption>
</figure>
