...
(However, the expectation is that an estimate via sampling will not be very different from the validation loss simply because sampling-based approaches do not seem to be working well for estimating joint predictive distributions.)
...
Further, the choice of the cutoff parameter $$m$$ in CLML and its impact on the results could be explored---while the authors mention that larger values of $$m$$ generally lead to better performance, it would be helpful to have more guidance on how to select $$m$$ in practice. Likewise, a comparison of CLML with other model selection criteria beyond ML, such as cross-validation or information criteria like AIC and BIC, would provide additional context.

It is also important to note that from a Bayesian perspective, using the model evidence (LML) is only valid for model selection when the model prior is uniform (i.e., all other things are considered equal). The Bayesian view on model selection given data $$\mathcal{D}$$ is:

$$
p(\mathcal{M}_i \mid \mathcal{D}) \propto p(\mathcal{D} \mid \mathcal{M}_i) \; p(\mathcal{M}_i).
$$

Hence, maximizing $$p(\mathcal{D} \mid \mathcal{M}_i)$$ is not sufficient depending on what model prior we use. In particular, the minimum description length (MDL) approach states that we want to select the model that minimizes both the length of describing its parameters and the data. By ignoring the model complexity when we only use the LML, we might potentially ignore an essential part of Occam's razor and the MDL principle. However, many other methods, including CLML and cross-validation, also ignore the model complexity and assume a uniform model prior. A uniform prior is valid, for example, when choosing specific hyperparameters. Moreover, estimating a model's description length is challenging and an open problem. The LML only makes sense for model selection when comparing models of equal complexity.
...
However, it is crucial to consider the broader context of model selection and the various criteria available. While CLML and validation loss have shown promising results, they are not the only options. Information criteria like AIC and BIC, as well as cross-validation, have their own merits and are widely used in practice. Each method has its assumptions and trade-offs, and the choice of the best criterion may depend on the specific application and data characteristics.

Moreover, the issue of ignoring model complexity and assuming uniform priors is not unique to CLML or validation loss. Many model selection methods, including cross-validation and information criteria, also make similar assumptions. While this simplification can be justified in certain contexts, such as when comparing models of equal complexity or choosing hyperparameters, it is essential to be aware of the limitations and potential consequences of ignoring model priors.

In conclusion, the paper's contribution lies in its rigorous examination of the LML's role in model selection, the introduction of CLML as an alternative, and the empirical results demonstrating its effectiveness for deep learning. However, further research is needed to explore the comparative advantages of CLML over other model selection criteria, investigate the impact of the cutoff parameter $$m$$, and address the challenges of incorporating model complexity and non-uniform priors in model selection. By considering these aspects and building upon the foundation laid by this paper, future work can advance our understanding of Bayesian model selection and its application to deep learning and beyond.
