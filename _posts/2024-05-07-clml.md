...
layout: distill
title: >-
  On the Marginal Likelihood and Cross-Validation: Understanding the Conditional Marginal Log-Likelihood (CLML) and Other Quantities for Model Selection
description: >-
  Bayesian model selection has long relied on the marginal likelihood and related quantities, often motivated by the principle of Occam's razor. 
  However, the suitability of these criteria depends on the specific context and goals of the modeling task. In this blog post, we challenge the conventional focus on the marginal likelihood for Bayesian model selection, arguing that the choice of model selection criterion should be guided by the desired outcomes rather than being seen as a direct consequence of Occam's razor. We introduce the concepts of log marginal likelihood (LML), cross-validation, and the recently introduced conditional log marginal likelihood (CLML), highlighting their connections and differences. Through thought experiments and empirical observations, we explore the behavior of these model selection criteria in various scenarios, finding that the conditional marginal cross-entropy, closely related to cross-validation, is often more reliable for optimizing generalization performance. We also review relevant literature comparing the CLML and validation loss for deep neural networks. By questioning the uniqueness of the (conditional) joint marginal likelihood and encouraging critical thinking about its foundations, we aim to foster a more nuanced understanding of Bayesian model selection.
date: 2024-05-07
future: true
htmlwidgets: true
tags:
