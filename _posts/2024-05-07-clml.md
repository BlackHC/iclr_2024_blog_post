...
The question on your mind probably is then: why bother with the marginal likelihood at all? And likewise, why bother with the conditional joint marginal cross-entropy and information? We could just always use leave-one-out cross-validation (i.e. the conditional marginal cross-entropy) or simply a validation loss and be done with it.

That is true, but the better question is: can we approximate the validation loss sooner than after training a model fully? Or can we do this in a way that is more efficient than performing inference on each element of a validation set?
...
Alternatively, when we train a model on a dataset for only a single epoch, which is still surprisingly common for large language models---and we do not perform any sort of active data sampling---the average training loss for each batch is a good approximation of the validation loss. With a cross-entropy loss, this is equivalent to an estimate of the conditional marginal cross-entropy.

Obviously, this might not be precise enough as the batch size might not be large enough, so we might want to average over the last few batches or use an exponential average as the training losses on the batches before the last one where trained with older model parameters. Compared to using the training loss of the last batch, this has a smoothing effect and is less sensitive to outliers.
