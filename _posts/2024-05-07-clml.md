...
Lyle et al. (2020)<d-cite key="lyle2020bayesian"></d-cite> in their paper "A Bayesian Perspective on Training Speed and Model Selection" propose using the sum of mini-batch training losses as a proxy for the LML to predict the generalization behavior of deep neural networks (DNNs). They argue that this sum, which corresponds to the area under the learning curve, can be used to select models that align with Occam's razor principle.

Building upon this work, Ru et al. (2021)<d-cite key="ru2021speedy"></d-cite> in "Speedy Performance Estimation for Neural Architecture Search" focus on using training speed estimators for model selection in the context of neural architecture search. They introduce a training speed estimator called TSE-E, which assigns higher weights to later epochs in the learning curve. 

The TSE-E estimator is similar in spirit to the CLML, as it discounts the initial terms of the learning curve and focuses on the later stages where the model has seen more data. Ru et al. demonstrate empirically that TSE-E has a higher correlation with the generalization performance of the final trained network compared to the full-length TSE estimator, which is a proxy for the LML.

The findings of Ru et al. (2021) support the idea behind the CLML proposed by Lotfi et al. (2022), suggesting that focusing on the later terms of the learning curve can provide a better indication of a model's generalization performance than the full LML.

These prior works provide valuable context and support for the ideas explored in "Bayesian Model Selection is not Model Selection" by Lotfi et al. (2022). They highlight the connections between the LML, cross-validation, and training speed estimators, and demonstrate the potential benefits of focusing on the later stages of the learning curve for model selection.
